    ...
    public partial class TarifGuncelle : Form
    {
        ...
        private void LoadRecipes()
        {
            ...
            string query = string.IsNullOrWhiteSpace(searchText)
                ? $"SELECT TarifID, TarifAdi, ResimYolu, HazirlamaSuresi, Kategori, Talimatlar FROM Tarifler {sortQuery}"
                : $"SELECT TarifID, TarifAdi, ResimYolu, HazirlamaSuresi, Kategori, Talimatlar FROM Tarifler WHERE TarifAdi LIKE @searchText {sortQuery}";
                    ...
                    using (SQLiteCommand command = new SQLiteCommand(query, connection))
                    {
                        ...
                        using (SQLiteDataReader reader = command.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                {
                                    TarifGuncellemeFormu tarifEkleGuncelleForm = new TarifGuncellemeFormu(
                                        tarifID,
                                        tarifAdi,
                                        hazirlamaSuresi,
                                        kategori,
                                    ...
                                    tarifEkleGuncelleForm.Show();
                                    this.Close();
                                };

                                cardPanel.Controls.Add(pictureBox);
                                cardPanel.Controls.Add(nameLabel);
                                cardPanel.Controls.Add(updateButton);
